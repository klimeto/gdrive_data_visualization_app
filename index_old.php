...
<html lang="en-us">
	<head>
		...
		<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
		 <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAU-n744ZptSQod9oCSmv8py8hrL1T0a-M&callback=drawRegionsMap" type="text/javascript"></script>
		<style>
		@media screen {
				#printSection {
					display: none;
	...
	<body>
		<center>
			<h1>Impact of Membrane Transporters</h1>
			<div>
				<h3>Geographic distribution of the questionnaire results</h3>
                                <h5>Results per country are accessible by clicking on a marker</h5>
				<div id="regions_div" style="width: auto; height: auto;"></div>
			</div>
		</center>
		<!--<button type="button" class="btn btn-info pull-right pull-top" onclick="alert('YOU CLICKED ME!')">Overal Results</button>-->
				<!-- Modal -->
		<div id="myModal" class="modal fade" role="dialog">
		 ...
		 <h5 style="float: right;">Developed by <a href="http://klimeto.com" target="_new"><span class="label label-default">klimeto</span></a></h5>
	...
	<script>
			...
			var out = [];
			var table;
			var first = ["Country", "Number of Respondends"];
			out.push(first);
			$.getJSON(url, sendData, function (res) {
					var vec = 0;
					var total = res.feed.entry.length;
					console.log('TOTAL ENTRIES SI: ', total);
					$.each(res.feed.entry, function (key, feature) {
						console.log(feature.gsx$inwhichcountrydoyoucurrentlywork.$t);
					...
					var ol = Object.keys(zgrupnute);
					var total_grouped = ol.length;
					console.log(total_grouped);
					$.each(zgrupnute, function (key, group) {
						var second = [key, group.length];
						out.push(second);
						vec = vec + 1;
						console.log(vec);
						if (vec == total_grouped){
            ...
            function drawRegionsMap() {
				...
				var chart = new google.visualization.GeoChart(document.getElementById('regions_div'));
					...
					console.log('DATA LENGTH:',  Object.keys(out).length);
					/*
					console.log(data);
					var options = {
						colorAxis: {
						//values:[1, 2, 3, 4, 5, 6, 26],
						colors: ['blue','yellow','green','red','pink']},
					...
					*/
					 var options = {
						region: 'world',
						displayMode: 'markers',
						colorAxis: {colors: ['blue','yellow','green','red','pink']}
					  };
...
var sendData = {};
			...
			//setTimeout(drawRegionsMap, 1500);
			/**setTimeout(function() {
				document.getElementById("loaderDIV").style.display="none";
			}, 1500);
				**/
